...
function GetStats(objectives){
	var doc = {};
	var stats = {base:0, intermediate:0, senior:0, baseTotal:0, intermediateTotal:0, seniorTotal:0, 
	...
	doc = objectives.reduce(function(prev, curr, idx, arr){
			if(curr.gate_level==""){
				prev.baseTotal += curr.score;
				if(curr.isMet){
					prev.base += curr.score;
				}
			}
			if(curr.gate_level=="intermediate"){
				prev.intermediateTotal += curr.score;
				if(curr.isMet){
					prev.intermediate += curr.score;
				}
			}
			if(curr.gate_level=="senior"){
				prev.seniorTotal += curr.score;
				if(curr.isMet){
					prev.senior += curr.score;
				}
			}
			["Communication", "Leadership", "Interpersonal", "Conflict", "Citizenship"].forEach(function(competency){
				if(curr[competency] == "Y"){
					prev[competency + "Total"] += curr.competency_weighting;
					if(curr.isMet){
						prev[competency] += curr.competency_weighting;
					}
				} 
			});
			return prev;
